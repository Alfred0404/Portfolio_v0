<script setup></script>

<template>
  <ul class="justify-left mt-10 flex flex-row items-center gap-10">
    <li>
      <a target="_blank" href="https://github.com/Alfred0404"
        ><img height="40px" width="40px" src="/github.png" alt="github logo"
      /></a>
    </li>
    <li>
      <a target="_blank" href="https://www.linkedin.com/in/alfred-de-vulpian/"
        ><img
          height="40px"
          width="40px"
          src="/linkedin.png"
          alt="linkedin logo"
      /></a>
    </li>
    <li>
      <a target="_blank" href="mailto:alfreddevulpian04@gmail.com"
        ><img height="40px" width="40px" src="/mail.png" alt="mail logo"
      /></a>
    </li>
    <li>
      <button download="CV_Alfred_de_Vulpian_Anglais.pdf">
        <svg
          width="40px"
          height="40px"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          stroke="#ffffff"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
          ></g>
          <g id="SVGRepo_iconCarrier">
            <path
              d="M15 15.5C14.7164 14.3589 13.481 13.5 12 13.5C10.519 13.5 9.28364 14.3589 9 15.5M12 9.5H12.01M13 9.5C13 10.0523 12.5523 10.5 12 10.5C11.4477 10.5 11 10.0523 11 9.5C11 8.94772 11.4477 8.5 12 8.5C12.5523 8.5 13 8.94772 13 9.5ZM7 21H17C18.1046 21 19 20.1046 19 19V5C19 3.89543 18.1046 3 17 3H7C5.89543 3 5 3.89543 5 5V19C5 20.1046 5.89543 21 7 21Z"
              stroke="#ffffff"
              stroke-width="1"
              stroke-linecap="round"
              stroke-linejoin="round"
            ></path>
          </g>
        </svg>
      </button>
    </li>
  </ul>
</template>
